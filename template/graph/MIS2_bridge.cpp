...
struct Graph{
	...
	if( maxd >= 6 ){
	    ...
	    link.solve();
	    if( link.bridge.size() != 0 ){
	    	mp select = link.bridge[0];
	    	//後々半分に分けることを考える.
	    	/*for(auto i:link.bridge){
	    	}*/
	    	ll u = change[select.first];
	    	ll v = change[select.second];
	    	set<ll> NU = Neighbor( rest , u );
	    	NU.insert(u);
	    	set<ll> NV = Neighbor( rest , v );
	    	NV.insert(v);
	    	vector<ll> next1,next2,next3,next4;//1: u select 2: v select 3:u discard 4: v discard
	    	for(auto i:rest ){
			if( NU.find(i) == NU.end() ) next1.push_back( i );
			if( NV.find(i) == NV.end() ) next2.push_back( i );
			if( i != u ) next3.push_back( i );
			if( i != v ) next4.push_back( i );
	    	}
	    	ll su = MIS(next1);
	    	ll sv = MIS(next2);
	    	ll du = MIS(next3);
	    	ll dv = MIS(next4);
	    	return max( du + dv , max( su + dv , sv + du ) );
	    }
