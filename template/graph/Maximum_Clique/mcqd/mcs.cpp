...
struct Maxclique {
  ...
  struct ColorClass {
    ...
    bool find(int p ){
	for(int j=0;j<size();j++)if( i[j] == p ) return true;
	return false;
    }
    void rewind() { sz = 0; };
    int size() const { return sz; }
...
//
//cutc が大きく　C[~]がまだ使われていない pcol > cutc
  ...
  for(int k1 = 1; k1 < cutc ; k1++){
    ...
    if( qq.size() == 1 ){
	  ...
	  C[pcol].erase_p( p );
          ...
          C[k1].push( p );
          C[k2].push( q );
	  //if( C[pcol].find( p ) ) cout<<"pcol don't erase"<<endl;
	  //if( C[k1].find( q ) )cout<<"k1 don't erase"<<endl;
	  //if( !C[k1].find( p ) ) cout<<"k1 don't insert"<<endl;
	  //if( !C[k2].find( q ) ) cout<<"k2 don't insert"<<endl;
          //cout<<"RE_COLOR "<<p<<' '<<q<<endl;
	  //cout<<"cutc,pcol,k1,k2 "<<cutc<<' '<<pcol<<' '<<k1<<' '<<k2<<endl;
	  return;
        }
      }
    }
...
void Maxclique::NUMBER_SORT_R( Vertices &Vs , Vertices &R , map<int,int> &No){
  ...
  int kmax = 1;
  int cutc = QMAX.size() - Q.size();
  //for(int i=0;i<=R.size();i++)C[i].rewind();
  C[1].rewind();
  C[2].rewind();
  ...
  for(int i=0;i<Vs.size();i++){
    ...
    // == RE-COLOR start ==
    if( k > cutc && k == kmax){
	/*cout<<"BEFORE "<<cutc<<' '<<i<<endl;
	for(int j=0;j<=kmax;j++){
	    cout<<j<<": ";
	    for(int l = 0;l<C[j].size();l++)cout<<C[j].at(l)<<' ';
	    cout<<endl;
	}*/
      RE_COLOR( p.i , k );
	...
	*/
    ...
    if( k <= cutc || !C[k].find( p.i ) ){ //前半の部分　とりあえずすぐに使わなそうな点
	R[ii++].i = p.i;
    }
    // == RE-COLOR end ==
  }
  ...
  // i != 0 -> i != -1
  //if( ii-1 !=  i ) cout<<"MMM: "<<ii<<' '<<i<<endl;//これはとりあえず大丈夫そう(成立していると思う)
  if( i != -1 ) No[ R[i].i ] = cutc;
  /*if( ii != -1 ){
      cout<<"Maybe MISS : "<<ii<<endl;
  }*/
  //for(int i=0;i<R.size();i++)cout<<"("<<R[i].i<<","<<No[R[i].i]<<") ";
  //for(int i=0;i<R.size();i++)cout<<R[i].i<<" ";
  //cout<<endl;
  //cout<<endl;
  /*set<int> check;
  for(int i=0;i<R.size();i++)if( !check.insert( R[i].i ).second ){
      cout<<"ONAZIMONOGAARUYO "<<R[i].i<<' '<<cutc<<endl;
      break;
  }
  check.clear();*/
  //if(i!=-1)cout<<i<<endl;
...
void Maxclique::MCS(vector<int> &maxclique, int &maxsize){
  ...
  //for(int i=0;i<R.size();i++)cout<<R[i].i<<' ';
  //cout<<endl;
  /*for(int i=0;i<Rp.size();i++)cout<<Rp[i].i<<' ';
  cout<<endl;*/
  /*for(int i=0;i<R.size();i++) cout<<"("<<R[i].i<<","<<No[R[i].i]<<") ";
  cout<<endl;*/
  expand( R , Rp , No );
...
void Maxclique::expand(Vertices &Vs , Vertices &R , map<int,int> &No){
  int cutc = QMAX.size()-Q.size();
  //cout<<Q.size()<<' '<<R.size()<<' '<<QMAX.size()<<endl;
  while( R.size() ){
...
// NUMBER_SORT_R 
// 294~295 for(int k = kmax ; k >= cutc + 1 ; k-- ) => for(int k = kmax ; k >= 0 ; k-- )
// RE_COLOR erase(p) => erase_p(p)

// NUMBER_SORT_R部分 これでとりあえず今のデータでは大きく遅くなったりしていないしある程度分岐数まで同じになっているので良さそう
    /*if( k <= cutc || !C[k].find( p.i ) ){ //前半の部分　とりあえずすぐに使わなそうな点
	R[ii++].i = p.i;
    }*/
