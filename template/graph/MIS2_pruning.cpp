...
struct Graph{
    ...
    ll MIS(vector<ll> &rest , ll nowcost = inf){//nowcost :今までとった個数
	ll res = 0;
	if(rest.size() == 0) return 0;
	
	if( Ans >= nowcost + rest.size() ) return -inf;//これ以上点を得ても無駄
	...
	if( mind <= 1 ){
	    ...
	    ll R =  res.first + MIS( res.second , nowcost + res.first );
	    return MANS( R ); 
	}
	...
	if( sub.size() != rest.size() ){
	    ...
	    ll R = MIS(sub , nowcost + subrest.size() );//枝刈りでまちがえて削られないようにsubrest を全てとったことにする.
	    R +=  MIS(subrest , nowcost + R);// nowcost今までとった個数と
	    return MANS( R );
	}
	...
	if( mind == 2 ){
		...
		if( N2.size() == 1 ){//Case2_2_1
		    ...
		    ll R =  max( res[0].first + MIS( res[0].second , nowcost + res[0].first ) , res[1].first + MIS( res[1].second , nowcost + res[1].first ) );
		    return MANS( R );
		}else{//Case2_2_2
		    set<ll> MV = Mirrors( rest , mini ,1);
		    vector<ll> next1 = Case_select( rest , mini ) , next2 = Case_discard( rest , MV );
		    ll R = max( 1 + MIS( next1 , nowcost + 1 ) , MIS( next2 , nowcost ) );
		    return MANS( R );
		} 
	    }
	}
	...
	if( mind == 3 ){
		...
		if( !MV.empty() ){
		    ...
		    ll R =  max( max( 1 + MIS( next1 , nowcost + 1 ) , 2 + MIS( nextt[0] , nowcost + 2 ) ),  max(2+ MIS( nextt[1] , nowcost + 2 ) , 2 + MIS( nextt[2] , nowcost + 2 ) ) );
		    return MANS( R );
		}
	...
	if( maxd == 5 && mind == 4 ){
	    auto res = Case6( rest );
	    ll R = max( max( res[0].first + MIS( res[0].second , nowcost + res[0].first) , res[1].first + MIS( res[1].second , nowcost + res[1].first ) ) , res[2].first + MIS( res[2].second ,nowcost + res[2].first ) );
	    return MANS( R );
	}
	/*
