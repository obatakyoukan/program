...
struct Graph{
    ...
    vector<ll> d;
    set<ll> articulation;//一度関節点になったらその後も関節点???
  
    hash<bitset< MAXSIZE > > hash_fn;
    map<ll,ll> result;//Hashで結果を残す
    map<ll,ll> result_cnt;//Hash を消すタイミング 0 delete
    queue< ll > result_del;//
    ll SubHash(vector<ll> &rest){
	bitset< MAXSIZE > bs(0);
	for(auto i:rest) bs.set(i);
	//ll res = 0;
	//for(auto i:rest ) res |= (1<<i);
	//return res;
	return hash_fn(bs);
    }
    void ResultDel(){
	ll hsh = result_del.front();
	result_del.pop();
	result_cnt[hsh]--;
	if( result_cnt[hsh] == 0 ){
	    result.erase( hsh );
	    result_cnt.erase( hsh );
	}
    }
    ll Register(ll hsh,ll ans ){//前提として持っていない
	result[ hsh ] = ans;
	result_cnt[ hsh ] ++;
	result_del.push( hsh );
	if( result_del.size() > RESULT_SIZE ) ResultDel();
	return ans;
    }
    ll ResultFind(ll hsh ){
	if( result.find( hsh ) != result.end() ){
	    result_cnt[hsh]++;
	    result_del.push(hsh);
	    if( result_del.size() > RESULT_SIZE ) ResultDel();
	    return result[hsh];
    ...
    ll MIS(vector<ll> &rest){// branching している部分はメモしていく
	ll res = 0;
	...
	if( mind == 2 ){
		...
		if( N2.size() == 1 ){//There is some miss in this range
		    ...
		    return Register( hsh , max( 2 + MIS( next1 ) , 2 + MIS( next2 ) ) );
		}else{
		    ...
		    return Register( hsh , max( 1 + MIS( next1 ) , MIS( next2 ) ) );
		} 
	    }
	}
	...
	if( mind == 3 ){
		...
		if( !MV.empty() ){
		    ...
		    return Register( hsh , max( max( 1 + MIS( next1 ) , 2 + MIS( nextt[0] ) ),  max(2+ MIS( nextt[1] ) , 2 + MIS( nextt[2] ) ) ) );
		}
	...
	if( sub.size() != rest.size() ){
	    ...
	    return Register( hsh , MIS(sub) + MIS(subrest) );
	...
	if( maxd >= 6 ){
	    ...
	    //articulationを取り除く
	    ll v = -1;
	    for(auto i:rest)if( articulation.find( v ) != articulation.end() )v = i;
	    vector<ll> fix;
	    if( v == -1 ){
	    	//bridge 多分　Connext の後がい
	   	 Lowlink link( rest.size() );
	    	map<ll,ll> change;
	    	for(ll i=0;i<rest.size();i++){
			change[i] = rest[i];
			for(ll j=i+1;j<rest.size();j++){
		    		if( mat[ rest[i] ][ rest[j] ] ){
					link.add_edge( i , j );
		    		}
			}
	    	}
		...
		NV.insert( v );
		for(auto i:rest){
		    if( NV.find( i ) == NV.end() ) next1.push_back( i );
		    if( i != v ) next2.push_back( i );
		}
		ll res = max( 1 + MIS( next1 ) , MIS( next2 ) );
		for( auto i:fix ) articulation.erase( i );
		return Register(hsh , res );
		//return res;
	    }
	...
	if( maxd == 5 && mind == 4 ){
	    ...
	    return Register( hsh , max( max( 1 + MIS( next1 ) , 1 + MIS( next2 ) ) , MIS( next3 ) ) );
