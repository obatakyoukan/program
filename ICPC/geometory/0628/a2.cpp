...
int main(){
 	while(1){
		...
		vector<vector<point> > pp(n+1,vector<point>(2,point(0,0) ) );
		pp[0][0] = pp[0][1] = c[0].c;
		pp[n][0] = pp[n][1] = c[1].c;
		vector<line> lines(n-1);
		for(int i=1;i<n;i++){
			pp[i] = crosspointcc(c[i],c[i-1]);
			lines[i-1] = line(pp[i][0],pp[i][1]);
		}
		dp[0][0] = dp[0][1] = 0.0;
		for(int i=1;i<=n;i++){
			for(int j=0;j<2;j++){
			    	dp[i][j] = min(dp[i-1][0] + (pp[i][j]-pp[i-1][0] ).abs() , dp[i-1][1] + (pp[i][j]-pp[i-1][1] ).abs() );
				for(int k=i-2;k>=0;k--){
					//line l = line( pp[k+1][0],pp[k+1][1] ) ;
					line l1 = line(pp[i][j],pp[k][0] );
					line l2 = line(pp[i][j],pp[k][1] );
					bool f1 = true;
					bool f2 = true;
					for(int lm =  k+1;lm < i ;lm++){
						if( !intersect( lines[k] , l1 ) ) f1 = false;
						if( !intersect( lines[k] , l2 ) ) f2 = false;
					}
					if(f1) dp[i][j] = min(dp[i][j],dp[k][0] + (pp[i][j] - pp[k][0]).abs() );
					if(f2) dp[i][j] = min(dp[i][j],dp[k][1] + (pp[i][j] - pp[k][1]).abs() );
					//if(!f1 && !f2) break;
				}
			}
		}
