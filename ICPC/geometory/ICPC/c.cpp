...
bool suc(vector<mp> &ps,double r){
    	int n = ps.size();
	polygon p ;
	vector<circle> c(n);
	for(int i=0;i<n;i++)p.push_back(ps[i].first);
	for(int i=0;i<n;i++)c[i] = circle( ps[i].first , ps[i].second * r);
	//for(int i=0;i<n;i++)cout<<c[i].c.x<<' '<<c[i].c.y<<' '<<c[i].r<<endl;
	for(int i=0;i<n;i++){
		for(int j=i+1;j<n;j++){
		    	int state = intersect(c[i],c[j]);
			if(state==0 || state == 4) continue;
			//if( (c[i].c-c[j].c).abs() <= c[i].r + c[j].r + EPS && (c[i].c -c[j].c).abs() >= abs(c[i].r-c[j].r)-EPS ){
			if(state>=1&&state<=3){
				polygon cc = crosspointcc( c[i],c[j] );
				for(int k=0;k<2;k++) p.push_back(cc[k]);
				/*if( abs(cc[0]-c[i].c) == c[i].r && abs(cc[1]-c[j].c)==c[j].r){
				}else{
				    	cc[0].print();
					cc[1].print();
					c[i].c.print();
					c[j].c.print();
					cout<<(cc[0]-c[i].c).abs()<<' '<<(cc[0]-c[i].c).abs()<<' '<<(cc[1]-c[i].c).abs()<<' '<<(cc[1]-c[j].c).abs()<<endl;
					cout<<c[i].r<<' '<<c[j].r<<endl<<endl;
					cout<<"ee"<<endl;
				}*/
			}
		}
	...
	for(int i=0;i<p.size();i++){
	    	bool f = true;
		for(int k=0;k<n;k++){
			if( abs(c[k].c - p[i]) >  c[k].r+ EPS ) {
			 //   c[k].c.print();
			 //   p[i].print();
			 //   cout<<(c[k].c-p[i]).abs()<<' '<<c[k].r<<endl;
			    f = false;
			    break;
			}
		}
...
int main(){
		...
		cout<< fixed << setprecision(10) <<(s+t)/2<<endl;
	}
	return 0;
}
