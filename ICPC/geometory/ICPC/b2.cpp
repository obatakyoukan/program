...
int main(){
		...
		vector<vector<mp> > angle(n);
		double res = 0.0;
		for(int i=0;i<n;i++){
		    	bool f = false;
			for(int j=0;j<n;j++){
				if(i==j)continue;
				//if( c[i].r < c[j].r && (intersect(c[i],c[j]) == ICC_CONTAIN || intersect(c[i],c[j]) == ICC_INSCRIBE )  ){
				 if(c[i].r <= c[j].r && abs(c[i].r-c[j].r) >= abs(c[i].c-c[j].c) ){
				    f = true;
				//    cout<<i<<' '<<c[i].r<<' '<<j<<' '<<c[j].r<<' '<<intersect(c[i],c[j]) <<endl;
				    break;
				}
				if( intersect(c[i],c[j]) == ICC_INTERSECT ){
					polygon cc = crosspointcc(c[i],c[j]);
					if( abs(cc[1]-cc[0] ) < EPS ) continue;
					point a = cc[0]-c[i].c , b = cc[1]-c[i].c;
					//if( cross(cc[j]-c[i].c,b) <  -EPS ) swap(a,b);
					//if( cross(a,b) < -EPS )swap(a,b);
					if( a.arg() >=   b.arg()   ){
						angle[i].push_back( mp( a.arg() , PI)   );
						angle[i].push_back( mp( -PI , b.arg() ) );
					}else{
						angle[i].push_back( mp( a.arg(),b.arg() ) );
					}
				}
			}
			if(f)continue;
			...
			for(int j=0;j<angle[i].size();j++){
				tmp +=  max( (angle[i][j].first - now), 0.0 );
				//cout<<i<<' '<<j<<' '<<angle[i][j].first<<' '<<angle[i][j].second<<' '<<tmp<<' '<<res<<endl;
				now = max(now,angle[i][j].second );
			}
			tmp += max( PI-now,0.0);
			res += c[i].r * tmp ;
		}
